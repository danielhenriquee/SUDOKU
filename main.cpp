...
int main() {
  ...
  int principal[TAM][TAM] = {
      4, 9, 5, 2, 8, 7, 3, 6, 1, 7, 2, 8, 6, 1, 3, 4, 9, 5, 3, 6, 1, 9, 5, 4, 7, 2, 8, 6, 5, 3, 8, 4, 9, 2, 1, 7, 9, 8, 4, 1, 7, 2, 6, 5, 3, 2, 1, 7, 5, 3, 6, 9, 8, 4, 1, 3, 2, 4, 6, 5, 8, 7, 9, 5, 4, 6, 7, 9, 8, 1, 3, 2, 8, 7, 9, 3, 2, 1, 5, 4, 6}; // Matriz principal
  ...
  do {
    ...
    case 1: // Jogar
      ...
      do { // Começo do jogo em si, quando o usuário começa a interagir
        ...
        for (i = 0; i < TAM; i++) {
          if (i == 3 or i == 6 ) { // Printa essa linha após a 3ª e a 6ª linha da matriz
            cout << "╠══════╬══════╬═════╣\n";
          }
          for (j = 0; j<TAM; j++) {
            ...
            if (jogo[i][j] == 0 and j != 8) { // Se o valor for 0, printa vazio com espaço depois
              cout << "  ";
            } else if (jogo[i][j] == 0 and j == 8) { // Se o valor for 0 na última coluna, printa vazio sem espaço depois
              cout << " ";
            } else if (jogo[i][j] != 0 and j < 8) { // Se tiver valor e não for a último coluna, printa o valor com espaço depois
              cout <<jogo[i][j] << " ";
        ...
        } else if ((auxi < 1 or auxi > 9 or auxj < 1 or auxj > 9) and (aux < 1 or aux > 9)) { // Confere se a posição é inválida E o valor é inválido
          cout << "Você digitou uma posição inválida e também um valor inválido! Digite apenas números naturais de 1 até 9.\n\n";
        } else if (auxi < 1 or auxi > 9 or auxj < 1 or auxj > 9) { // Confere se a posição é inválida
        ...
        } else if (aux < 1 or aux > 9) { // Confere se o valor é inválido
          cout << "Você digitou um valor inválido! Digite um número natural de 1 até 9.\n\n";
        } else if (aux != gabarito[auxi - 1][auxj - 1] and jogo[auxi - 1][auxj - 1] == 0) { // Confere se o valor é incorreto
